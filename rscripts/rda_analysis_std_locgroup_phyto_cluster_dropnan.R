...
df <- read_excel("MERGED_DATA_INTERPOLATED_FINAL2.xlsx", sheet="MERGE")
View(df)

n_species <-  df |>
  select("Agl":(ncol(df) - 1)) |> 
  ncol()
n_species

# checks to see how many only nan values for species we have
allnas_cond <- rowSums(is.na(select(df, "Agl":(ncol(df) - 1)))) == n_species
allnas_rows <- df |> 
  filter(allnas_cond)
nrow(allnas_rows)
...
# eliminates species that were measured under 1% of time in cluster
for (name in names(groups)) {
  groups[[name]] <- groups[[name]] |> 
    select(where(~ mean(is.na(.)) <= 0.99))
}
...
spe.rda <- rda(spec.hel[["Coastal-Estuary"]] ~ . + Condition(loc), abio_std[["Coastal-Estuary"]])
print(summary(spe.rda))
perc <- round(100 * (summary(spe.rda)$cont$importance[2, 1:dim]), 2)
sc_sp <- scores(spe.rda, display = "species", choices = c(1:dim), scaling = scaling)

spec_clusters <- list(
  "cluster_0" = c('Ezo', 'Gde', 'Osi', 'Cdeb', 'Ptr', 'Lun', 'Cda'),
  "cluster_1" = c('Omo', 'Ram', 'Rse', 'Tec', 'Dle', 'Etr', 'Gfl', 'Gsp', 'Nsc', 'Pbi', 'Pbr', 'Stu', 'Kgl', 'Oro', 'Tor', 'Cdi', 'Cra', 'Ore', 'Ata', 'Cfu', 'Cgr', 'Lan', 'Pcl', 'Pmi', 'Pos', 'Pse', 'Cden', 'Aco', 'Dip', 'Csu', 'Mnu', 'Pco', 'Cwa', 'Pba', 'Dac', 'Nsi', 'Rst', 'Pst', 'Acn', 'Ccu', 'Pan', 'Gfa', 'Hta', 'Dsp', 'Psu', 'Cei', 'Ndi', 'Dro', 'Cha', 'Pac', 'Cau', 'Coc', 'Pte', 'Mpe', 'Pde', 'Plo', 'Dpu', 'Rte', 'Fja', 'Hak', 'Dno'),
  "cluster_2" = c('Agl', 'Dbr', 'Oau', 'Tle', 'Tni', 'Tro', 'Tno'),
  "cluster_3" = c('Orh', 'Pha', 'Edu', 'Mhe')
)
...
print(sc_sp)
sc_bp <- scores(spe.rda, display = "bp", choices = c(1:dim), scaling = scaling)
