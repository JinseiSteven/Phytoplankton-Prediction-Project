...
df <- read_excel("MERGED_DATA_180624.xlsx", sheet="MERGE_FINAL")
View(df)
...
# drop 7 abio na rows
df_filtered <- df_filtered |> 
  filter(if_all("LOC_CODE": "SRP:SI", ~!is.na(.)))
nrow(df_filtered)

# should be "IM [Jm2d]" !!!!!
abio <- df_filtered |> 
  select("LOC_CODE":"SRP:SI")
View(abio)
...
# Scaling 1
# Calculate goodness-of-fit for each species
spec.good <- goodness(spec.rda)

# Select species with goodness-of-fit at least 0.6 on the first two axes
sel.sp <- which(spec.good[, 1] >= 0.1 | spec.good[, 2] >= 0.1)
